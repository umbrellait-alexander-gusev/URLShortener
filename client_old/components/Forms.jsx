...
const RandomlyForm = CreateReactClass({
    ...
    handleRandomlyChange(event) {
        ...
        const shortUrl = apiPrefix + '/' + randomStringFun(5);
    ...
    handleCustomChange(event) {
        if (event.target.value.match("^[a-zA-Z0-9_]*$") !== null && event.target.value.length < 6) {
            this.setState({shortCustomUrl: event.target.value});
        }

        const shortUrl = apiPrefix + '/' + event.target.value.slice(0, 5);
